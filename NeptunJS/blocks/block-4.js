...
+(function () {
	function _0x13E1B(_0x13E1B) {
		...
		var _0x13E47 = (
			(_0x13E1B.clientX - bar.offset().left) /
			bar.outerWidth()
		).toFixed(2);
		nepVolume = _0x13E47;
	...
	function _0x13F7B() {
		...
		if (
			jscd.browser === "Microsoft Edge" ||
			(jscd.browser === "Chrome" && jscd.browserMajorVersion <= 60) ||
			(jscd.browser === "Firefox" && jscd.browserMajorVersion <= 56) ||
			(jscd.browser === "Opera" && jscd.browserMajorVersion <= 44)
		) {
	...
	function _0x140AF(_0x13E73) {
		...
		RA.numBuffers =
			((_0x13E73 * RA.context.sampleRate) / (1e3 * RA.BUFFER_SIZE)) | 0;
		if (RA.numBuffers < 2) {
			RA.numBuffers = 2;
		}
		for (var _0x13E47 = 0; _0x13E47 < RA.numBuffers; _0x13E47++) {
			RA.buffers[_0x13E47] = RA.context.createBuffer(
				2,
				RA.BUFFER_SIZE,
	...
	_psdqaq78 = _0x14133;
	if (netp === 1) {
		if (neptunPlatform === null || ltm === false) {
		} else {
			bar = Neptq("#" + NCSS + "bar");
